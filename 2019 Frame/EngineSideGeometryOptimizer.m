function [CoordMatrix,Weight,Stiffness,Ratio,OptimizedFrameData]=EngineSideGeometryOptimizer(refFrame,Seed,NumGens,GenSize,numSurv,maxChangeX,maxChangeY,maxChangeZ)
...
while k < (NumGens+1)
    ...
    for j=1:numSurv
          ...
          TWorking(:,j) = GetEngineSideTorsionalStiffness(testSeed);
          WWorking(:,j) = GetWeight(testSeed);
    end
    for i=1:numSurv
        
        [CM2,WM2,TM2]=Iterator(refFrame.Coord',Seed,CSWorking(:,:,i),GenSize,numSurv,symmetricalNodes,fixedNodes,fixedPosX,fixedNegX,fixedPosY,fixedNegY,fixedPosZ,fixedNegZ,maxChangeX,maxChangeY,maxChangeZ);
...
Weight=WSWorking(:,1);
OptimizedFrameData = EngineSideGeometryAndLoadingForGeometryOptimizers_2019Frame(Seed.A',CoordMatrix');
end
...
function [CM2,SW2,TS2]=Iterator(refFrameCoord,Seed,CSWorking,G,S,symmetricalNodes,fixedNodes,fixedPosX,fixedNegX,fixedPosY,fixedNegY,fixedPosZ,fixedNegZ,maxChangeX,maxChangeY,maxChangeZ)
...
for g=1:G
    ...
    newFrameData = EngineSideGeometryAndLoadingForGeometryOptimizers_2019Frame(Seed.A',CSWorking');
    TS = GetEngineSideTorsionalStiffness(newFrameData);
