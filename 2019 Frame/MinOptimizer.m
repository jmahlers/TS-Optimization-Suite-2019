function [OptimizedFrameData,FrontSideStiffness,EngineSideStiffness] = MinOptimizer(m,n,TargetTS,plotThicknesses,plotGraphs,squareTubes)
...
[initialTS,initialWeight,initialRatio] = TestFrame(min,m,squareTubes,toggle);
TSStorage(1)=initialTS;WeightStorage(1)=initialWeight;RatioStorage(1)=initialRatio;
IntMatrixStorage(1,:)=min;

% Printing initial values
PrintRoundInfo(TSStorage(1),WeightStorage(1),RatioStorage(1),counter,toggle);

% Begin main loop
counterOffset = 0;
while TSStorage(counter) < TargetTS || TSStorage(counter - counterOffset) < TargetTS
    ...
    while (1==1)
        for i=1:m-15
            if min(i) < numberOfTubeThicknessOptions
               ...
               [currTS,currWeight,currRatio] = TestFrame(min,m,squareTubes,toggle);
    ...
    [roundBestTS,roundBestWeight,roundBestRatio]=TestFrame(min,m,squareTubes,toggle);
    ...
    PrintRoundInfo(TSStorage(counter),WeightStorage(counter),RatioStorage(counter),counter,toggle);
end
...
if plotThicknesses
    ...
    if toggle
        FrontSideStiffness =  TSStorage(counter);
        EngineSideStiffness = TSStorage(counter-counterOffset);
    else
        FrontSideStiffness =  TSStorage(counter-counterOffset);
        EngineSideStiffness = TSStorage(counter);
    end
    counterOffset = 1;
    OptimizedFrameData = GeometryAndLoadingForThicknessOptimizers_2019Frame(IntToThicknessMatrix(IntMatrixStorage(counter,:),m,squareTubes));
end
    ...
    function [TS, Weight, Ratio] = TestFrame(intMatrix,m,squareTubes,toggle)
        tubeThicknessMatrix = IntToThicknessMatrix(intMatrix,m,squareTubes);
        if toggle
            FrameData = GeometryAndLoadingForThicknessOptimizers_2019Frame(tubeThicknessMatrix);
    ...
    function PrintRoundInfo(TS,Weight,Ratio,counter,toggle)
        if toggle
            fprintf("\nFront-Side Round #"+counter+"\n");
            fprintf("Torsional Stiffness:\t"+TS+" N*m/deg\n");
            fprintf("Associated Weight:\t\t"+Weight+" lbs\n");
            fprintf("Ratio (TS/Weight):\t\t"+Ratio+"\n\n");
        else
            fprintf("\nEngine-Side Round #"+counter+"\n");
            fprintf("Torsional Stiffness:\t"+TS+" N*m/deg\n");
            fprintf("Associated Weight:\t\t"+Weight+" lbs\n");
            fprintf("Ratio (TS/Weight):\t\t"+Ratio+"\n\n");
        end
